...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'createUser') {
    ...
    $sql = "INSERT INTO niveles (nivel) VALUES (?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $nivel);

    if ($stmt->execute()) {
        echo json_encode(["status" => "success", "message" => "Nivel creado exitosamente."]);
        echo "<script>alert('nivel creado exitosamente.');</script>";
...
<body>
    <div class="container">
        <header>
            ...
            <h1 >Modulo de Creacion de Niveles</h1>
        </header>
        <form action="crearnivel.php" method="post">
